...
namespace Nd20TemplateLib
{
    ...
    public class AttackStat
    {
        ...
        public string ToStringRTFMonster()
        {
            string returnString = "";
        ...
        private string PrintAttackBody(bool FullAttackOrNot)
        {
            ...
            string meleeWeapons = PrintAttacksByGroup(AttackType.MELEE, FullAttackOrNot);

            string rangedWeapons = PrintAttacksByGroup(AttackType.RANGED, FullAttackOrNot);

            if (!String.IsNullOrEmpty(meleeWeapons) && !String.IsNullOrEmpty(rangedWeapons))
            {
                returnString = meleeWeapons + " or " + rangedWeapons;
            }
            else if (!String.IsNullOrEmpty(meleeWeapons))
            {
                returnString = meleeWeapons;
            }
            else if (!String.IsNullOrEmpty(rangedWeapons))
            {
                returnString = rangedWeapons;
            }
        ...
        private string PrintAttacksByGroup(AttackType AttackTypeToPrint, bool FullAttackOrNot)
        {
            ...
            foreach (CharWeapon Weapon in CharacterWeapons)
            {
                ...
                {
                    if (FullAttackOrNot == false && isFirstWeapon == false)
                    {
                        returnString += " or ";
                    }
                    else if(isFirstWeapon == false)
                    {
                        returnString += " and ";
                    }

                    if(isFirstWeapon == true)
                    {
                        isFirstWeapon = false;
                    }
        ...
        public string PrintAttackType(AttackType MyAttack, bool FullAttackOrNot, CharWeapon WeaponToPrint)
        {
            ...
            if (true == FullAttackOrNot && WeaponToPrint == null)
            {
                FullAttackCount = GetMinimum((int)((BAB-1) / 5) + 1, MAX_ATTACK_COUNT);
            }
            else if (true == FullAttackOrNot && WeaponToPrint.WeaponCategory == WeaponCategory.Human)
            {
                FullAttackCount = GetMinimum((int)((BAB - 1) / 5) + 1, MAX_ATTACK_COUNT);
            }
            else
            ...
            switch (MyAttack)
            {
                case AttackType.MELEE:
                    ...
                    if (WeaponToPrint != null && WeaponToPrint.WeaponCategory == WeaponCategory.Secondary)
                    {
