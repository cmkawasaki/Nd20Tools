...
using Nd20StatblockGenerator.TemplateInfo;
...
namespace Nd20TemplateLib
{
    public class NPCTemplate
    {
        ...
        public string RacialFeats = "";
        public int BonusSkillPoints = 0;

        public List<CharWeapon> RacialWeapons = new List<CharWeapon>();
        ...
        SummonMetadata creatureMetadata = null;

        public int summonCategory = 0;
        ...
        public void PrintTemplate()
        {
            ...
            PrintCharacterTitle(CharFile, "");

            PrintCoreStatblock(CharFile);

            int SummonLevel = (ClassStatEntry.GetTotalLevel(ClassEntries) / 2);
            int skillPoints = Math.Max(SummonLevel, (SummonLevel * (BaseStats.IntMod + 2)));
            int CharFeats;

            switch(this.summonCategory)
            {
                case 0:
                    CharFeats = 0;
                    skillPoints = Math.Max(SummonLevel, (SummonLevel * (BaseStats.IntMod + 2)));
                    break;
                case 1:
                    CharFeats = (ClassStatEntry.GetTotalLevel(ClassEntries) / 4) + 1 + BonusFeats;
                    skillPoints = Math.Max(SummonLevel, (SummonLevel * (BaseStats.IntMod + 3)));
                    break;
                case 2:
                    CharFeats = (ClassStatEntry.GetTotalLevel(ClassEntries) / 3) + 1 + BonusFeats;
                    skillPoints = Math.Max(SummonLevel, (SummonLevel * (BaseStats.IntMod + 4)));
                    break;
                default:
                    CharFeats = 0;
                    skillPoints = Math.Max(SummonLevel, (SummonLevel * (BaseStats.IntMod + 2)));
            ...
            CharFile.WriteLine(" (" + skillPoints + " skill points)");

            CharFile.WriteBold("Feats: ");
            if (!String.IsNullOrEmpty(RacialFeats))
            {
                CharFile.Write(RacialFeats + ",");
            }
        ...
        {
            if (this.creatureMetadata != null)
            {
                CharFile.WriteBold(CharacterName + " (Level " + (this.GetCurrentLevel()/2) + " " + this.creatureMetadata.ClassTitle + " " + this.raceName + ")");
            }
            else
            {
        ...
        private void PrintCoreStatblock(RTFWriter CharFile)
        {
            ...
            if (this.creatureMetadata != null)
            {
                CharFile.Write("; " + CharacterSize.ReturnSizeText(CharSize) + " " + this.creatureMetadata.CreatureType.ToLower());
            }

            if (MyCharType.GetTitleText() != null && MyCharType.GetTitleText().Length > 0)
            {
                CharFile.Write(" " + MyCharType.GetTitleText().ToLower());
            }
            ...
            CharFile.Write(MovementType.PrintMovementTypes(MovementTypes));
            CharFile.Write("; ");
            ...
            TemplateAttacks.CharacterWeapons = RacialWeapons;
        ...
        public void ConsumeTemplate(NPCTemplate TemplateToBeConsumed)
        {
            ...
            if (!String.IsNullOrEmpty(RacialFeats) && !String.IsNullOrEmpty(TemplateToBeConsumed.RacialFeats))
            {
                RacialFeats = RacialFeats + ", " + TemplateToBeConsumed.RacialFeats;
            }
            if (String.IsNullOrEmpty(RacialFeats) && !String.IsNullOrEmpty(TemplateToBeConsumed.RacialFeats))
            {
                RacialFeats = TemplateToBeConsumed.RacialFeats;
            }
            ...
            if (TemplateToBeConsumed.RacialWeapons != null && TemplateToBeConsumed.RacialWeapons.Count > 0)
            {
                if (RacialWeapons == null)
                {
                    RacialWeapons = new List<CharWeapon>();
                }

                foreach (CharWeapon weapon in TemplateToBeConsumed.RacialWeapons)
                {
                    RacialWeapons.Add(weapon);
                }
            }

            if (TemplateToBeConsumed.MovementTypes != null && TemplateToBeConsumed.MovementTypes.Count > 0)
            {
                ...
                foreach (MovementType type in TemplateToBeConsumed.MovementTypes)
                {
        ...
        public static void CreateItemAndPrintEx(CharacterStats BaseStats, string[] ClassList, int[] LevelList, string FileName, int PUCount, int PowerRankCount, int BloodlineChosen, IRace RaceType, ICharacterType CharType, ICharacterType BunshinType, int MaxStrRank, int MaxSpeedRank, int summonCategory)
        {
            ...
            MyTemplate.summonCategory = summonCategory;
        ...
        private static NPCTemplate CreateCharacterStatblock3(CharacterStats BaseStats, string[] ClassList, int[] LevelList, string lFilename, IRace RaceType)
        {
            ...
            Character.raceName = RaceType.GetTitleText();

            for (int i = 0; i < ClassList.Length; i++)
            {
                if (ClassList[i] != null)
                {
                    SummonMetadataList XMLClassList = SummonMetadataList.Initialize();

                    foreach (SummonMetadata metadata in XMLClassList.SummonList)
                    {
                        if (metadata.SummonClass.ClassTitle == ClassList[i])
                        {
                            Character.creatureMetadata = metadata;
                            Character.ConsumeTemplate(metadata.SummonClass.CreateClassTemplate(LevelList[i], Character.GetCurrentLevel() + 1, Character.GetCurrentLevel() + LevelList[i], Character.BaseStats, Character.BonusSkillPoints));
                            break;
                        }
                    }
                }
